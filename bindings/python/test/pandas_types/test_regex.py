...
from pymongoarrow.pandas_types import PandasRegex, PandasRegexArray
...
def make_datum():
    return Regex(str(np.random.rand()))
...
@pytest.fixture
def data(dtype):
    return PandasRegexArray(np.array(make_data(), dtype=object), dtype=dtype)


@pytest.fixture
def data_missing(dtype):
    return PandasRegexArray(np.array([np.nan, make_datum()], dtype=object), dtype=dtype)
